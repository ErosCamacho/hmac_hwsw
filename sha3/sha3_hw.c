...
void sha3_ms2xl_256(unsigned long long int* a, unsigned long long int* b, PYNQ_MMIO_WINDOW ms2xl, unsigned long long int pos_pad, int pad, int DBG) {
	...
	if (pad == 1) {
		...
		for (int i = 0; i < (SIZE_BLOCK_SHA3_256 / 64); i++) {
	...
	if (pad) {
		...
		if (DBG == 3) printf(" i_max: %f %f %d\n\r", ((double)SIZE_SHA3_256 / (double)64), ceil((double)SIZE_SHA3_256 / (double)64), (int)ceil((double)SIZE_SHA3_256 / (double)64));

		for (int i = 0; i < (int)ceil((double)SIZE_SHA3_256 / (double)64); i++) {
			reg_addr = (unsigned long long int)(i);
			PYNQ_writeMMIO(&ms2xl, &reg_addr, ADDRESS, sizeof(unsigned long long int));
			PYNQ_readMMIO(&ms2xl, &reg_data_out, DATA_OUT, sizeof(unsigned long long int));
			b[i] = reg_data_out;
...
void sha3_ms2xl_512(unsigned long long int* a, unsigned long long int* b, PYNQ_MMIO_WINDOW ms2xl, unsigned long long int pos_pad, int pad, int DBG) {
	...
	if (pad == 1) {
		...
		for (int i = 0; i < (SIZE_BLOCK_SHA3_512 / 64); i++) {
	...
	if (pad) {
		...
		if (DBG == 3) printf(" i_max: %f %f %d\n\r", ((double)SIZE_SHA3_512 / (double)64), ceil((double)SIZE_SHA3_512 / (double)64), (int)ceil((double)SIZE_SHA3_512 / (double)64));

		for (int i = 0; i < (int)ceil((double)SIZE_SHA3_512 / (double)64); i++) {
			reg_addr = (unsigned long long int)(i);
			PYNQ_writeMMIO(&ms2xl, &reg_addr, ADDRESS, sizeof(unsigned long long int));
			PYNQ_readMMIO(&ms2xl, &reg_data_out, DATA_OUT, sizeof(unsigned long long int));
			b[i] = reg_data_out;
...
void sha3_hw_256(unsigned char* in, unsigned char* out, unsigned long long int length, PYNQ_MMIO_WINDOW ms2xl, int DBG) {
	...
	unsigned long long int buffer_in[(SIZE_BLOCK_SHA3_256 / 64)];
	unsigned long long int buffer_out[(int)ceil((double)SIZE_SHA3_256 / (double)64)];
	unsigned long long int buf_1, buf_2;

	unsigned long long tic = 0, toc;

	// ------- Number of hash blocks ----- //
	hb_num = ((length + 3) / SIZE_BLOCK_SHA3_256) + 1; //3 bits for padding
	pos_pad = length % SIZE_BLOCK_SHA3_256;
	...
	// ------- Operation ---------------- //
	for (unsigned long long int hb = 1; hb <= hb_num; hb++) {
		ind = (hb - 1) * (SIZE_BLOCK_SHA3_256 / 8);
		for (int i = 0; i < (SIZE_BLOCK_SHA3_256 / 64); i++) {
			if ((ind + 0) * 8 >= length) in[ind + 0] = 0x00;
			if ((ind + 1) * 8 >= length) in[ind + 1] = 0x00;
			if ((ind + 2) * 8 >= length) in[ind + 2] = 0x00;
		...
		if (hb == hb_num)						last_hb = 1;
		else if (pos_pad > (SIZE_BLOCK_SHA3_256 - 4))	last_hb = 2; // Add a new block with mix padding
		else									last_hb = 0;
		...
		sha3_ms2xl_256(buffer_in, buffer_out, ms2xl, pos_pad, last_hb, DBG);
	}
...
void sha3_hw_512(unsigned char* in, unsigned char* out, unsigned long long int length, PYNQ_MMIO_WINDOW ms2xl, int DBG) {
	...
	unsigned long long int buffer_in[(SIZE_BLOCK_SHA3_512 / 64)];
	unsigned long long int buffer_out[(int)ceil((double)SIZE_SHA3_512 / (double)64)];
	unsigned long long int buf_1, buf_2;

	unsigned long long tic = 0, toc;

	// ------- Number of hash blocks ----- //
	hb_num = ((length + 3) / SIZE_BLOCK_SHA3_512) + 1; //3 bits for padding
	pos_pad = length % SIZE_BLOCK_SHA3_512;
	...
	// ------- Operation ---------------- //
	for (unsigned long long int hb = 1; hb <= hb_num; hb++) {
		ind = (hb - 1) * (SIZE_BLOCK_SHA3_512 / 8);
		for (int i = 0; i < (SIZE_BLOCK_SHA3_512 / 64); i++) {
			if ((ind + 0) * 8 >= length) in[ind + 0] = 0x00;
			if ((ind + 1) * 8 >= length) in[ind + 1] = 0x00;
			if ((ind + 2) * 8 >= length) in[ind + 2] = 0x00;
		...
		if (hb == hb_num)						last_hb = 1;
		else if (pos_pad > (SIZE_BLOCK_SHA3_512 - 4))	last_hb = 2; // Add a new block with mix padding
		else									last_hb = 0;
		...
		sha3_ms2xl_512(buffer_in, buffer_out, ms2xl, pos_pad, last_hb, DBG);
	}
