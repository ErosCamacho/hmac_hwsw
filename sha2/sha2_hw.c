...
void sha2_ms2xl_init_sha2_256(PYNQ_MMIO_WINDOW ms2xl, unsigned long long int length, int DBG) {
	unsigned long long int op;
	...
	reg_addr = (unsigned long long int)(0);
	reg_data_in = (unsigned long long int)(length >> WIDTH_SHA2_256);
	PYNQ_writeMMIO(&ms2xl, &reg_addr, ADDRESS, sizeof(unsigned long long int));
	PYNQ_writeMMIO(&ms2xl, &reg_data_in, DATA_IN, sizeof(unsigned long long int));
...
void sha2_ms2xl_init_sha2_512(PYNQ_MMIO_WINDOW ms2xl, unsigned long long int length, int DBG) {
	unsigned long long int op;
	...
	reg_addr = (unsigned long long int)(0);
	reg_data_in = (unsigned long long int)(length >> WIDTH_SHA2_512);
	PYNQ_writeMMIO(&ms2xl, &reg_addr, ADDRESS, sizeof(unsigned long long int));
	PYNQ_writeMMIO(&ms2xl, &reg_data_in, DATA_IN, sizeof(unsigned long long int));
...
void sha2_ms2xl_256(unsigned long long int* a, unsigned long long int* b, unsigned long long int length, PYNQ_MMIO_WINDOW ms2xl, int last_hb, int DBG) {
	...
	for (int i = 0; i < (SIZE_BLOCK_SHA2_256 / WIDTH_SHA2_256); i++) {
	...
	if (last_hb) {
		...
		if (DBG == 3) printf(" i_max: %f %f %d\n\r", ((double)SIZE_SHA2_256 / (double)WIDTH_SHA2_256), ceil((double)SIZE_SHA2_256 / (double)WIDTH_SHA2_256), (int)ceil((double)SIZE_SHA2_256 / (double)WIDTH_SHA2_256));

		for (int i = 0; i < (int)ceil((double)SIZE_SHA2_256 / (double)WIDTH_SHA2_256); i++) {
			reg_addr = (unsigned long long int)(i);
			PYNQ_writeMMIO(&ms2xl, &reg_addr, ADDRESS, sizeof(unsigned long long int));
			PYNQ_readMMIO(&ms2xl, &reg_data_out, DATA_OUT, sizeof(unsigned long long int));
			b[i] = reg_data_out;
...
void sha2_ms2xl_512(unsigned long long int* a, unsigned long long int* b, unsigned long long int length, PYNQ_MMIO_WINDOW ms2xl, int last_hb, int DBG) {
	...
	for (int i = 0; i < (SIZE_BLOCK_SHA2_512 / WIDTH_SHA2_512); i++) {
	...
	if (last_hb) {
		...
		if (DBG == 3) printf(" i_max: %f %f %d\n\r", ((double)SIZE_SHA2_512 / (double)WIDTH_SHA2_512), ceil((double)SIZE_SHA2_512 / (double)WIDTH_SHA2_512), (int)ceil((double)SIZE_SHA2_512 / (double)WIDTH_SHA2_512));

		for (int i = 0; i < (int)ceil((double)SIZE_SHA2_512 / (double)WIDTH_SHA2_512); i++) {
			reg_addr = (unsigned long long int)(i);
			PYNQ_writeMMIO(&ms2xl, &reg_addr, ADDRESS, sizeof(unsigned long long int));
			PYNQ_readMMIO(&ms2xl, &reg_data_out, DATA_OUT, sizeof(unsigned long long int));
			b[i] = reg_data_out;
...
void sha2_hw_256(unsigned char* in, unsigned char* out, unsigned long long int length, PYNQ_MMIO_WINDOW ms2xl, int DBG) {

	unsigned long long int hb_num;
	unsigned long long int ind;
	int last_hb = 0;

	unsigned long long int buffer_in[(SIZE_BLOCK_SHA2_256 / WIDTH_SHA2_256)];
	unsigned long long int buffer_out[(int)ceil((double)SIZE_SHA2_256 / (double)WIDTH_SHA2_256)];
	unsigned long long int buf_1, buf_2;

	unsigned long long tic = 0, toc;

	// ------- Number of hash blocks ----- //
	hb_num = (unsigned long long int)((length + (2 * (unsigned long long int)WIDTH_SHA2_256)) / (unsigned long long int)SIZE_BLOCK_SHA2_256) + 1; //3 bits for padding
	...
	sha2_ms2xl_init_sha2_256(ms2xl, length, DBG);

	// ------- Operation ---------------- //
	if (WIDTH_SHA2_256 == 64) {
		for (unsigned long long int hb = 1; hb <= hb_num; hb++) {
			ind = (hb - 1) * (SIZE_BLOCK_SHA2_256 / 8);
			for (int i = 0; i < (SIZE_BLOCK_SHA2_256 / WIDTH_SHA2_256); i++) {
				if ((ind + 0) * 8 >= length) in[ind + 0] = 0x00;
				if ((ind + 1) * 8 >= length) in[ind + 1] = 0x00;
				if ((ind + 2) * 8 >= length) in[ind + 2] = 0x00;
			...
			if (hb == hb_num) last_hb = 1;
			sha2_ms2xl_256(buffer_in, buffer_out, length, ms2xl, last_hb, DBG);
		}
	}
	else {
		for (unsigned long long int hb = 1; hb <= hb_num; hb++) {
...
void sha2_hw_512(unsigned char* in, unsigned char* out, unsigned long long int length, PYNQ_MMIO_WINDOW ms2xl, int DBG) {

	unsigned long long int hb_num;
	unsigned long long int ind;
	int last_hb = 0;

	unsigned long long int buffer_in[(SIZE_BLOCK_SHA2_512 / WIDTH_SHA2_512)];
	unsigned long long int buffer_out[(int)ceil((double)SIZE_SHA2_512 / (double)WIDTH_SHA2_512)];
	unsigned long long int buf_1, buf_2;

	unsigned long long tic = 0, toc;

	// ------- Number of hash blocks ----- //
	hb_num = (unsigned long long int)((length + (2 * (unsigned long long int)WIDTH_SHA2_512)) / (unsigned long long int)SIZE_BLOCK_SHA2_512) + 1; //3 bits for padding
	...
	sha2_ms2xl_init_sha2_512(ms2xl, length, DBG);

	// ------- Operation ---------------- //
	if (WIDTH_SHA2_512 == 64) {
		for (unsigned long long int hb = 1; hb <= hb_num; hb++) {
			ind = (hb - 1) * (SIZE_BLOCK_SHA2_512 / 8);
			for (int i = 0; i < (SIZE_BLOCK_SHA2_512 / WIDTH_SHA2_512); i++) {
				if ((ind + 0) * 8 >= length) in[ind + 0] = 0x00;
				if ((ind + 1) * 8 >= length) in[ind + 1] = 0x00;
				if ((ind + 2) * 8 >= length) in[ind + 2] = 0x00;
		...
		for (unsigned long long int hb = 1; hb <= hb_num; hb++) {
			...
			if (hb == hb_num) last_hb = 1;
			sha2_ms2xl_512(buffer_in, buffer_out, length, ms2xl, last_hb, DBG);
		}
	}
