/******************************************************************************/
/*
 *  IMSE.CNM_SPIRS-HwSec_puf4r4_2.2: puf4r4.c    32/64-bits
 *
...
int PUF_createMMIOWindow(MMIO_WINDOW* puf, int baseaddr, int length, int DBG) {
   ...
   if (Status != SUCCESS) {
      printf(" createMMIOWindow failed!\n\n");
      exit(ERROR);
   }
 #endif
 ...
 #if defined(STANDALONE)
   dataout = PUF4R4_mReadReg(*puf, DATAOUT);
 #else
 ...
 ******************************************************************************/
 ...
 int PUF_writeChallegesMask(MMIO_WINDOW* puf, int puf_selr[], int regs, int DBG) {
  ...
  // send reset & configuration
   ...
   #if defined(AXI64)
         if (puf_selr[chl_r] != 0) printf("%16.16llX ", puf_selr[chl_r]);
   #else
         if (puf_selr[chl_r] != 0) printf("%8.8X ", puf_selr[chl_r]);
   #endif			
         }
      printf("\n");				  
      }
   ...
   for (int chl_r=0; chl_r<regs; chl_r++) {
      op = chl_r; 
      PUF4R4_mWriteReg(*puf, CHLADDC, op);
      dat = puf_selr[chl_r];
      PUF4R4_mWriteReg(*puf, CHLDATA, op);
      op = op + WCHL;
      PUF4R4_mWriteReg(*puf, CHLADDC, op);	 
   }
   op = 0;
 ...
 ******************************************************************************/
 ...
 #else
   op = RESET;
   PUF4R4_mWriteReg(*puf, CONTROL, op);
   op = OPER;
   PUF4R4_mWriteReg(*puf, CONTROL, op);
   op = (cmps<<7) + config;
   PUF4R4_mWriteReg(*puf, CONTROL, op);
...
int PUF_readOutput(MMIO_WINDOW* puf, int cmps, unsigned int puf_out[], int DBG) {
 ...
 #if defined(STANDALONE)
      PUF4R4_mWriteReg(*puf, PUFADDR, pufaddr);
      pufout = PUF4R4_mReadReg(*puf, PUFOUT);
 #else
