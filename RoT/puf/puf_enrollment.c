/******************************************************************************/
/*
 *  IMSE.CNM_SPIRS-HwSec_puf4r4_2.2: puf_enrollment.c for PUF4R4_2.2  32/64-bits
 *
 ...
 *                 the Test System: "_PR.txt" files include the PUF reference 
 *                 outputs, while "_CM.txt" files store the selection masks for 
 *                 each enrollment stage:
 *
 *          SPIRS_RoT_<board_ID>_<puf>_<t_code>_XX.txt
*/
/******************************************************************************/
...
int main(int argc, char** argv) {
   ...
   for (int s=0; s<ns; s++) {
      ...
      if (OUT_FILE == 1) {      
         ...
         strcat(output_file1, "_PR.txt");      
         strcat(output_file2, "_CM.txt");      
      ...
      for (int r=1; r<=N_runs; r++) {    // Generate r runs
      //for (int r=N_runs; r<=N_runs; r++) {  // Generate only run r
