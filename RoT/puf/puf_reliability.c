/*****************************************************************************/
/*
 *  IMSE.CNM_SPIRS-HwSecpuf4r4_2.2: puf_reliability.c for PUF4R4_2.2  32/64-bits
 *
   ...
   for (int s=0; s<ns; s++) {
      ...
      for (int run=1; run<=N_runs; run++) {
         ...
         for (t=1; t<=N_tests; t++) {
		    ...
		    if (last_reg < ceil((float)(N_CMPS-E_CMPS)/(Dbw/4.))) {
 			   printf("\n  last_reg: %d  ", last_reg);
               triesr++;
               t = t-1;
			   if (triesr > 5) {
            ...
            //Read PUF results
            int Status = PUF_readOutput(&puf, N_CMPS-E_CMPS, puf_output, 0);
            if (Status == ERROR) {
               printf("failed!\n");
            }
            if (OUT_FILE == 1) {
               for (int r=0; r<ceil((float)(N_CMPS-E_CMPS)/(Dbw/4.)); r++) {
         #if defined(AXI64)
