/*****************************************************************************/
/*
 *  IMSE.CNM_SPIRS-HwSecpuf4r4_2.2: puf_uniqueness.c for PUF4R4_2.2  32/64-bits
 *
 *  Created on: 07/09/2023
 *      Author: santiago@imse-cnm.csic.es
 *
 *  Function:
 *   - Evaluates the uniqueness of the PUFs instantiated in a Test System. 
 ...
 *     successive series of invocations to the other PUFs are analyzed.
 *
 *  Outputs:
 *   - DBG = 0 --> Tests failed or passed test for each PUF and each run
 ...
 *
 *          SPIRS_RoT_<board_ID>_<puf>_<t_code>_<n_run>.txt
*/
 /*****************************************************************************/																	   
...
int RC         = 25;       // -p ECC Repetition code
...
int main(int argc, char** argv) {
   ...
   N_pufs = sizeof(opt)/sizeof(opt[0]);      // Number of PUFs in Test System
 #if defined(AXI64)
   N_regs = ceil(N_CMPS/16.);     	// Number of registers 
 #else
   N_regs = ceil(N_CMPS/8.);     	// Number of registers 
 #endif
            ...
            for (int run=1; run<=N_runs; run++) {
              ...
              tr = Wtime();
               ...
               for (t=1; t<=N_tests; t++) {
		          ...
		          if (last_reg < ceil((float)N_CMPS/(Dbw/4.))) {
 			         printf("\n  last_reg: %d  ", last_reg);
                     triesr++;
                     t = t-1;
			         if (triesr > 5) {
                     ...
                     //Read PUF results
                     int Status = PUF_readOutput(&puf, N_CMPS, puf_output, 0);
                     if (Status == ERROR) {
                        printf("failed!\n");
                     } 
                        
                     if (OUT_FILE == 1) {
                        for (int r=0; r<ceil((float)N_CMPS/(Dbw/4.)); r++) {
            #if defined(AXI64)			 
                     ...
                     // Get PUF output for N_cmps-E_cmps
                     if (E_CMPS != 0) {		 	  
                        int cr = 0;
                        for (int c=0; c<N_CMPS; c++) {
                 	       if (puf_selm[c] == 0) {
               		          puf_outc[cr] = puf_outc[c];
   	         		          cr++;
   	         		       }
                        }
   	                 }
               ...
               if(passed_t != 0) {
                  printf("  Failed  #tests %5d", passed_t);
               } else {
                  printf("  Passed  #tests %5d", failed_t);
               }      
               if (DBG > 0) {         
